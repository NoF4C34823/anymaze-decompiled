
void FUN_1408d7900(longlong param_1,longlong param_2,int param_3,ulonglong param_4,ulonglong param_5
                  ,uint *param_6,undefined8 param_7,int param_8)

{
  float *pfVar1;
  float *pfVar2;
  uint uVar3;
  float fVar4;
  float fVar5;
  undefined1 auVar6 [32];
  undefined1 auVar7 [32];
  longlong lVar8;
  longlong lVar9;
  ulonglong uVar10;
  ulonglong uVar11;
  ulonglong uVar12;
  ulonglong uVar13;
  undefined1 uVar14;
  int iVar15;
  ulonglong uVar16;
  ulonglong uVar17;
  ulonglong uVar18;
  ulonglong uVar19;
  undefined1 auVar20 [16];
  undefined1 auVar21 [32];
  undefined1 auVar22 [16];
  undefined1 auVar23 [32];
  undefined1 auVar24 [16];
  undefined1 auVar25 [32];
  undefined1 auVar26 [16];
  undefined1 auVar27 [32];
  longlong lStack_70;
  longlong lStack_68;
  longlong lStack_60;
  longlong lStack_58;
  longlong lStack_50;
  
  lVar8 = (longlong)param_3;
  uVar11 = 0;
  uVar3 = *param_6;
  lVar9 = lVar8 + -2;
  if (SCARRY8(lVar8,-2) != lVar8 + -2 < 0) {
    lVar9 = lVar8 + 3;
  }
  uVar10 = (longlong)(int)lVar9 + 1;
  fVar4 = (float)param_6[1];
  if (4 < (longlong)uVar10) {
    uVar10 = uVar11;
  }
  uVar13 = (longlong)(int)uVar10 + 1;
  fVar5 = (float)param_6[2];
  if (4 < (longlong)uVar13) {
    uVar13 = uVar11;
  }
  uVar17 = (longlong)(int)uVar13 + 1;
  if (4 < (longlong)uVar17) {
    uVar17 = uVar11;
  }
  uVar18 = (longlong)(int)uVar17 + 1;
  if (4 < (longlong)uVar18) {
    uVar18 = uVar11;
  }
  if (0 < (longlong)param_5) {
    if ((longlong)param_5 < 0x10) {
LAB_1408d813c:
      uVar16 = 0;
    }
    else {
      uVar12 = param_2 * 5 + param_1;
      if ((longlong)param_5 < 0x1d) {
        lStack_58 = param_2 * lVar9 + param_1;
        uVar16 = param_5 & 0xfffffffffffffff0;
        lStack_60 = param_2 * uVar18 + param_1;
        lStack_50 = param_2 * uVar10 + param_1;
        lStack_68 = param_2 * uVar17 + param_1;
        lStack_70 = param_2 * uVar13 + param_1;
      }
      else {
        uVar11 = uVar12 & 0x1f;
        if (uVar11 != 0) {
          if ((uVar12 & 3) != 0) goto LAB_1408d813c;
          uVar11 = 0x20 - uVar11 >> 2;
        }
        if ((longlong)param_5 < (longlong)(uVar11 + 0x10)) goto LAB_1408d813c;
        lStack_58 = param_2 * lVar9 + param_1;
        lStack_60 = param_2 * uVar18 + param_1;
        lStack_50 = param_2 * uVar10 + param_1;
        lStack_68 = param_2 * uVar17 + param_1;
        uVar16 = param_5 - (param_5 - uVar11 & 0xf);
        lStack_70 = param_2 * uVar13 + param_1;
        if (uVar11 != 0) {
          uVar19 = 0;
          do {
            auVar20 = vfmadd213ss_fma(ZEXT416((uint)(*(float *)(lStack_58 + uVar19 * 4) +
                                                    *(float *)(lStack_60 + uVar19 * 4))),
                                      ZEXT416(uVar3),
                                      ZEXT416((uint)(fVar5 * *(float *)(lStack_70 + uVar19 * 4))));
            auVar20 = vfmadd213ss_fma(ZEXT416((uint)(*(float *)(lStack_50 + uVar19 * 4) +
                                                    *(float *)(lStack_68 + uVar19 * 4))),
                                      ZEXT416((uint)fVar4),auVar20);
            *(int *)(uVar12 + uVar19 * 4) = auVar20._0_4_;
            uVar19 = uVar19 + 1;
          } while (uVar19 < uVar11);
        }
      }
      auVar23._4_4_ = uVar3;
      auVar23._0_4_ = uVar3;
      auVar23._8_4_ = uVar3;
      auVar23._12_4_ = uVar3;
      auVar23._16_4_ = uVar3;
      auVar23._20_4_ = uVar3;
      auVar23._24_4_ = uVar3;
      auVar23._28_4_ = uVar3;
      auVar21._4_4_ = fVar5;
      auVar21._0_4_ = fVar5;
      auVar21._8_4_ = fVar5;
      auVar21._12_4_ = fVar5;
      auVar21._16_4_ = fVar5;
      auVar21._20_4_ = fVar5;
      auVar21._24_4_ = fVar5;
      auVar21._28_4_ = fVar5;
      do {
        pfVar1 = (float *)(lStack_58 + uVar11 * 4);
        pfVar2 = (float *)(lStack_60 + uVar11 * 4);
        auVar27._0_4_ = *pfVar1 + *pfVar2;
        auVar27._4_4_ = pfVar1[1] + pfVar2[1];
        auVar27._8_4_ = pfVar1[2] + pfVar2[2];
        auVar27._12_4_ = pfVar1[3] + pfVar2[3];
        auVar27._16_4_ = pfVar1[4] + pfVar2[4];
        auVar27._20_4_ = pfVar1[5] + pfVar2[5];
        auVar27._24_4_ = pfVar1[6] + pfVar2[6];
        auVar27._28_4_ = pfVar1[7] + pfVar2[7];
        pfVar1 = (float *)(lStack_50 + uVar11 * 4);
        pfVar2 = (float *)(lStack_68 + uVar11 * 4);
        auVar6._4_4_ = fVar4 * (pfVar1[1] + pfVar2[1]);
        auVar6._0_4_ = fVar4 * (*pfVar1 + *pfVar2);
        auVar6._8_4_ = fVar4 * (pfVar1[2] + pfVar2[2]);
        auVar6._12_4_ = fVar4 * (pfVar1[3] + pfVar2[3]);
        auVar6._16_4_ = fVar4 * (pfVar1[4] + pfVar2[4]);
        auVar6._20_4_ = fVar4 * (pfVar1[5] + pfVar2[5]);
        auVar6._24_4_ = fVar4 * (pfVar1[6] + pfVar2[6]);
        auVar6._28_4_ = pfVar1[7] + pfVar2[7];
        auVar20 = vfmadd231ps_fma(auVar6,auVar27,auVar23);
        auVar20 = vfmadd231ps_fma(ZEXT1632(auVar20),auVar21,
                                  *(undefined1 (*) [32])(lStack_70 + uVar11 * 4));
        *(undefined1 (*) [32])(uVar12 + uVar11 * 4) = ZEXT1632(auVar20);
        pfVar1 = (float *)(lStack_58 + 0x20 + uVar11 * 4);
        pfVar2 = (float *)(lStack_60 + 0x20 + uVar11 * 4);
        auVar25._0_4_ = *pfVar1 + *pfVar2;
        auVar25._4_4_ = pfVar1[1] + pfVar2[1];
        auVar25._8_4_ = pfVar1[2] + pfVar2[2];
        auVar25._12_4_ = pfVar1[3] + pfVar2[3];
        auVar25._16_4_ = pfVar1[4] + pfVar2[4];
        auVar25._20_4_ = pfVar1[5] + pfVar2[5];
        auVar25._24_4_ = pfVar1[6] + pfVar2[6];
        auVar25._28_4_ = pfVar1[7] + pfVar2[7];
        pfVar1 = (float *)(lStack_50 + 0x20 + uVar11 * 4);
        pfVar2 = (float *)(lStack_68 + 0x20 + uVar11 * 4);
        auVar7._4_4_ = fVar4 * (pfVar1[1] + pfVar2[1]);
        auVar7._0_4_ = fVar4 * (*pfVar1 + *pfVar2);
        auVar7._8_4_ = fVar4 * (pfVar1[2] + pfVar2[2]);
        auVar7._12_4_ = fVar4 * (pfVar1[3] + pfVar2[3]);
        auVar7._16_4_ = fVar4 * (pfVar1[4] + pfVar2[4]);
        auVar7._20_4_ = fVar4 * (pfVar1[5] + pfVar2[5]);
        auVar7._24_4_ = fVar4 * (pfVar1[6] + pfVar2[6]);
        auVar7._28_4_ = pfVar1[7] + pfVar2[7];
        auVar20 = vfmadd231ps_fma(auVar7,auVar25,auVar23);
        auVar20 = vfmadd231ps_fma(ZEXT1632(auVar20),auVar21,
                                  *(undefined1 (*) [32])(lStack_70 + 0x20 + uVar11 * 4));
        *(undefined1 (*) [32])(uVar12 + 0x20 + uVar11 * 4) = ZEXT1632(auVar20);
        uVar11 = uVar11 + 0x10;
      } while (uVar11 < uVar16);
    }
    if (uVar16 + 1 <= param_5) {
      uVar11 = param_5 - uVar16;
      if ((longlong)uVar11 < 4) {
        uVar12 = 0;
      }
      else {
        uVar12 = uVar11 & 0xfffffffffffffffc;
        auVar22._4_4_ = uVar3;
        auVar22._0_4_ = uVar3;
        auVar22._8_4_ = uVar3;
        auVar22._12_4_ = uVar3;
        auVar20._4_4_ = fVar5;
        auVar20._0_4_ = fVar5;
        auVar20._8_4_ = fVar5;
        auVar20._12_4_ = fVar5;
        uVar19 = 0;
        do {
          pfVar1 = (float *)(param_2 * lVar9 + param_1 + uVar16 * 4 + uVar19 * 4);
          pfVar2 = (float *)(param_2 * uVar18 + param_1 + uVar16 * 4 + uVar19 * 4);
          auVar24._0_4_ = *pfVar1 + *pfVar2;
          auVar24._4_4_ = pfVar1[1] + pfVar2[1];
          auVar24._8_4_ = pfVar1[2] + pfVar2[2];
          auVar24._12_4_ = pfVar1[3] + pfVar2[3];
          pfVar1 = (float *)(param_2 * uVar10 + param_1 + uVar16 * 4 + uVar19 * 4);
          pfVar2 = (float *)(param_2 * uVar17 + param_1 + uVar16 * 4 + uVar19 * 4);
          auVar26._0_4_ = fVar4 * (*pfVar1 + *pfVar2);
          auVar26._4_4_ = fVar4 * (pfVar1[1] + pfVar2[1]);
          auVar26._8_4_ = fVar4 * (pfVar1[2] + pfVar2[2]);
          auVar26._12_4_ = fVar4 * (pfVar1[3] + pfVar2[3]);
          auVar24 = vfmadd231ps_fma(auVar26,auVar24,auVar22);
          auVar24 = vfmadd231ps_fma(auVar24,auVar20,
                                    *(undefined1 (*) [16])
                                     (param_2 * uVar13 + param_1 + uVar16 * 4 + uVar19 * 4));
          *(undefined1 (*) [16])(param_2 * 5 + param_1 + uVar16 * 4 + uVar19 * 4) = auVar24;
          uVar19 = uVar19 + 4;
        } while (uVar19 < uVar12);
      }
      if (uVar12 < uVar11) {
        do {
          auVar20 = vfmadd213ss_fma(ZEXT416((uint)(*(float *)(lVar9 * param_2 + param_1 + uVar16 * 4
                                                             + uVar12 * 4) +
                                                  *(float *)(uVar18 * param_2 + param_1 + uVar16 * 4
                                                            + uVar12 * 4))),ZEXT416(uVar3),
                                    ZEXT416((uint)(fVar5 * *(float *)(uVar13 * param_2 + param_1 +
                                                                      uVar16 * 4 + uVar12 * 4))));
          auVar20 = vfmadd213ss_fma(ZEXT416((uint)(*(float *)(uVar10 * param_2 + param_1 +
                                                              uVar16 * 4 + uVar12 * 4) +
                                                  *(float *)(uVar17 * param_2 + param_1 + uVar16 * 4
                                                            + uVar12 * 4))),ZEXT416((uint)fVar4),
                                    auVar20);
          *(int *)(param_2 * 5 + param_1 + uVar16 * 4 + uVar12 * 4) = auVar20._0_4_;
          uVar12 = uVar12 + 1;
        } while (uVar12 < uVar11);
      }
    }
  }
  if (param_8 == 0) {
    if ((longlong)param_5 < 1) {
      return;
    }
    if ((longlong)param_5 < 0x20) {
LAB_1408d81a8:
      uVar10 = 0;
    }
    else {
      if ((longlong)param_5 < 0x12c0) {
        uVar10 = param_5 & 0xffffffffffffffe0;
        uVar11 = 0;
        lVar9 = param_2 * 5 + param_1;
      }
      else {
        uVar11 = param_4 & 0x1f;
        if (uVar11 != 0) {
          uVar11 = 0x20 - uVar11;
        }
        if ((longlong)param_5 < (longlong)(uVar11 + 0x20)) goto LAB_1408d81a8;
        lVar9 = param_2 * 5 + param_1;
        uVar13 = 0;
        uVar10 = param_5 - (param_5 - uVar11 & 0x1f);
        if (uVar11 != 0) {
          do {
            iVar15 = (int)ROUND(*(float *)(lVar9 + uVar13 * 4));
            if (0xfe < iVar15) {
              iVar15 = 0xff;
            }
            uVar14 = (undefined1)iVar15;
            if (iVar15 < 1) {
              uVar14 = 0;
            }
            *(undefined1 *)(uVar13 + param_4) = uVar14;
            uVar13 = uVar13 + 1;
          } while (uVar13 < uVar11);
        }
      }
      do {
        auVar21 = vcvtps2dq_avx(*(undefined1 (*) [32])(lVar9 + uVar11 * 4));
        auVar23 = vcvtps2dq_avx(*(undefined1 (*) [32])(lVar9 + 0x20 + uVar11 * 4));
        auVar6 = vcvtps2dq_avx(*(undefined1 (*) [32])(lVar9 + 0x40 + uVar11 * 4));
        auVar7 = vcvtps2dq_avx(*(undefined1 (*) [32])(lVar9 + 0x60 + uVar11 * 4));
        auVar21 = vpackssdw_avx2(auVar21,auVar23);
        auVar23 = vpackssdw_avx2(auVar6,auVar7);
        auVar21 = vpermq_avx2(auVar21,0xd8);
        auVar23 = vpermq_avx2(auVar23,0xd8);
        auVar21 = vpackuswb_avx2(auVar21,auVar23);
        auVar21 = vpermq_avx2(auVar21,0xd8);
        *(undefined1 (*) [32])(uVar11 + param_4) = auVar21;
        uVar11 = uVar11 + 0x20;
      } while (uVar11 < uVar10);
    }
    if (param_5 < uVar10 + 1) {
      return;
    }
    param_5 = param_5 - uVar10;
    if ((longlong)param_5 < 4) {
      uVar11 = 0;
    }
    else {
      uVar11 = param_5 & 0xfffffffffffffffc;
      uVar13 = 0;
      do {
        auVar20 = vcvtps2dq_avx(*(undefined1 (*) [16])
                                 (param_2 * 5 + param_1 + uVar10 * 4 + uVar13 * 4));
        auVar20 = vpackssdw_avx(auVar20,auVar20);
        auVar20 = vpackuswb_avx(auVar20,auVar20);
        *(int *)(uVar13 + param_4 + uVar10) = auVar20._0_4_;
        uVar13 = uVar13 + 4;
      } while (uVar13 < uVar11);
    }
    if (param_5 <= uVar11) {
      return;
    }
    do {
      iVar15 = (int)ROUND(*(float *)(param_1 + param_2 * 5 + uVar10 * 4 + uVar11 * 4));
      if (0xfe < iVar15) {
        iVar15 = 0xff;
      }
      uVar14 = (undefined1)iVar15;
      if (iVar15 < 1) {
        uVar14 = 0;
      }
      *(undefined1 *)(uVar11 + param_4 + uVar10) = uVar14;
      uVar11 = uVar11 + 1;
    } while (uVar11 < param_5);
    return;
  }
  if ((longlong)param_5 < 1) {
    return;
  }
  if ((longlong)param_5 < 0x20) {
LAB_1408d81c4:
    uVar10 = 0;
    if (param_5 == 0) {
      return;
    }
  }
  else {
    uVar11 = param_4 & 0x1f;
    if (uVar11 != 0) {
      uVar11 = 0x20 - uVar11;
    }
    if ((longlong)param_5 < (longlong)(uVar11 + 0x20)) goto LAB_1408d81c4;
    lVar9 = param_2 * 5 + param_1;
    uVar13 = 0;
    uVar10 = param_5 - (param_5 - uVar11 & 0x1f);
    if (uVar11 != 0) {
      do {
        iVar15 = (int)ROUND(*(float *)(lVar9 + uVar13 * 4));
        if (0xfe < iVar15) {
          iVar15 = 0xff;
        }
        uVar14 = (undefined1)iVar15;
        if (iVar15 < 1) {
          uVar14 = 0;
        }
        *(undefined1 *)(uVar13 + param_4) = uVar14;
        uVar13 = uVar13 + 1;
      } while (uVar13 < uVar11);
    }
    do {
      auVar21 = vcvtps2dq_avx(*(undefined1 (*) [32])(lVar9 + uVar11 * 4));
      auVar23 = vcvtps2dq_avx(*(undefined1 (*) [32])(lVar9 + 0x20 + uVar11 * 4));
      auVar6 = vcvtps2dq_avx(*(undefined1 (*) [32])(lVar9 + 0x40 + uVar11 * 4));
      auVar7 = vcvtps2dq_avx(*(undefined1 (*) [32])(lVar9 + 0x60 + uVar11 * 4));
      auVar21 = vpackssdw_avx2(auVar21,auVar23);
      auVar23 = vpackssdw_avx2(auVar6,auVar7);
      auVar21 = vpermq_avx2(auVar21,0xd8);
      auVar23 = vpermq_avx2(auVar23,0xd8);
      auVar21 = vpackuswb_avx2(auVar21,auVar23);
      auVar21 = vpermq_avx2(auVar21,0xd8);
      auVar21 = vmovntdq_avx(auVar21);
      *(undefined1 (*) [32])(uVar11 + param_4) = auVar21;
      uVar11 = uVar11 + 0x20;
    } while (uVar11 < uVar10);
    if (param_5 < uVar10 + 1) {
      return;
    }
    param_5 = param_5 - uVar10;
    if (3 < (longlong)param_5) {
      uVar13 = 0;
      uVar11 = param_5 & 0xfffffffffffffffc;
      do {
        auVar20 = vcvtps2dq_avx(*(undefined1 (*) [16])(lVar9 + uVar10 * 4 + uVar13 * 4));
        auVar20 = vpackssdw_avx(auVar20,auVar20);
        auVar20 = vpackuswb_avx(auVar20,auVar20);
        *(int *)(uVar13 + param_4 + uVar10) = auVar20._0_4_;
        uVar13 = uVar13 + 4;
      } while (uVar13 < uVar11);
      goto LAB_1408d80dc;
    }
  }
  uVar11 = 0;
LAB_1408d80dc:
  if (uVar11 < param_5) {
    do {
      iVar15 = (int)ROUND(*(float *)(param_1 + param_2 * 5 + uVar10 * 4 + uVar11 * 4));
      if (0xfe < iVar15) {
        iVar15 = 0xff;
      }
      uVar14 = (undefined1)iVar15;
      if (iVar15 < 1) {
        uVar14 = 0;
      }
      *(undefined1 *)(uVar11 + param_4 + uVar10) = uVar14;
      uVar11 = uVar11 + 1;
    } while (uVar11 < param_5);
  }
  return;
}

