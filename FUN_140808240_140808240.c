...
void FUN_140808240(longlong param_1,float param_2,longlong param_3,undefined8 param_4)
  ...
  if (uVar6 < uVar7) {
    ...
    do {
      *(float *)(param_3 + in_RDX * 4 + uVar6 * 4) =
           param_2 * (float)((uint)*(ushort *)(param_1 + -2 + uVar6 * 2) +
                            (uint)*(ushort *)(param_1 + 2 + uVar6 * 2)) +
           in_XMM0_Da * (float)*(ushort *)(param_1 + uVar6 * 2);
      uVar6 = uVar6 + 1;
    } while (uVar6 < uVar7);
  }
  return;
